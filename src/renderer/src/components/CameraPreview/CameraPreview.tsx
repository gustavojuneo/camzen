import { Camera, Play, ScanFace, Square } from 'lucide-react'
    ...
    <Panel className="grid min-h-0 grid-rows-[auto_1fr_auto] overflow-hidden">
      ...
      <div className="flex items-center justify-between gap-3 border-t border-neutral-800 px-4 py-3">
        ...
        <div className="flex shrink-0 items-center gap-2">
          <Button
            intent={segmentationEnabled ? 'secondary' : 'ghost'}
            onClick={() => setSegmentationEnabled((current) => !current)}
          >
            <ScanFace className="h-4 w-4" />
            {segmentationEnabled ? 'Desativar segmentacao' : 'Ativar segmentacao'}
          </Button>
          <Button intent="ghost" onClick={virtualCam.loadModule}>
            Preparar v4l2
          </Button>
